...
export const MessageProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <MessageContext.Provider value={{ triggerMessage }}>
      <Messages messages={messages} onClose={removeMessage} />
      {children}
...
const Messages: React.FC<{ messages: Message[]; onClose: (id: number) => void }> = ({ messages, onClose }) => (
  <div style={{
    position: "fixed",
    top: "10px",
    width: "100%",
    ...
    {messages.map((message, index) => (
      <MessagePopup key={message.id} color={message.color} index={index} onClose={() => onClose(message.id)}>
        {message.message}
      </MessagePopup>
    ))}
  </div>
