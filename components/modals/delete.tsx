...
export const DeleteManyModal = ({ids, isOpen, setOpen, handleDelete, word} : {word:string, ids : string[], handleDelete : (id : string[]) => void, isOpen : boolean, setOpen : (state:boolean) => void}) => {
    return (
        <AnimatePresence>
            {isOpen && (
                <motion.div
                ...
                >
                    <motion.div
                        ...
                        onClick={(e) => e.stopPropagation()}
                    >
                        <h2 className="text-xl font-semibold mb-4 text-white">Delete {ids.length} {word}{ids.length > 1 && "s"}</h2>
                        <p className="text-white mb-6">Are you sure you want to delete these {ids.length} {word.toLowerCase()}{ids.length > 1 && "s"}?</p>
                        <div className="flex justify-end gap-4">
                            <button
                                onClick={() => setOpen(false)}
                                className="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition-colors"
